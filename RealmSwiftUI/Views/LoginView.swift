        ...
        NavigationView {
            VStack(alignment: .leading) {
                ...
                GeometryReader { geometry in
                    LAnimationView(name: "mood", loopMode: .loop)
                ...
                NavigationLink(destination: BaseView().navigationBarBackButtonHidden(true), isActive: $isSignedBefore)  {
                    NavigationLink(destination: IntroView().navigationBarBackButtonHidden(true), isActive: $isNavigated) {
                        Button(action: {
                            self.load.toggle()
                            ...
                            if(load == true){
                                ...
                                Text("Login")
                                    ...
                                    .background(RoundedRectangle(cornerRadius: 10).fill(LinearGradient(gradient: Gradient(colors: [.black, .indigo]), startPoint: .top, endPoint: .bottom)))
                            }
                        }
                        .padding(.top, 10)
                    }
